...
You can also find this as a [Python script in the SDK repo](https://github.com/HumanSignal/label-studio-sdk/blob/master/tutorials/export_snapshots.py). Note that in that example, you must first set the URL, API key, and project ID as environment variables. 
...
You can also use the following example to bulk export from a project without creating an snapshot first. 

```python

# Set your variables in the script as above 
# Or as environment variables. Must include a project ID. 
...
data = client.projects.exports.as_json(PROJECT_ID)
